...
> 分册：`Java-日志与注释规范.md` · `Java-外部调用规范.md` · `Java-Spring规范.md` · `Java-配置与工程规范.md` · `Java-SpringSecurity规范.md`（占位）。  
...
- XML 路径与 `module.*.mapper` 包对应（见架构文档）；复杂 SQL 按需加「为什么」注释（见日志与注释规范）。
...
- [ ] SQL：单表 MP+Lambda；联表 XML；**无二次单表内存关联**；无 N+1；Stream 无 IO  
- [ ] 外部调用 / Spring / 配置符合对应分册  
