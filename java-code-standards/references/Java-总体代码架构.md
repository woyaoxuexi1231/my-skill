...
1. **先有业务模块边界，层只存在于模块内部**：以业务主题切出模块（`order`、`user`…），**所有业务模块统一收在顶层 `modules/` 下**；一个类进哪个模块由**它服务哪块业务**决定；进了模块，它再按技术角色落 `controller / service / mapper / dto`。
...
5. **模块内默认平铺**：同层类按资源/能力命名直接放该层根包（`OrderController` 在 `modules/order/controller/`）；**不要**在模块内再套第二层业务子包——模块本身已是业务边界。
...
**一句话判断**：新人在 `modules/order/` 点一眼，能不能立刻找到"订单"相关的一切？能 → 业务边界清晰；不能 → 模块划分或命名有问题。
...
**全局与模块的分界**：`config / security / common / entity / mapper` 这些**全局基础设施平铺在基线包**；**所有业务模块一律收在 `modules/` 下**，一个业务主题一个子包。"全局还是 `modules/`"一问即定。

**按需取用**：`modules/order/…/query/`、`common/event/` 这类**可选子包**只在确有对应内容才建；空子目录不留。`entity`（表映射）与 `dto`（契约）分开放，二者不互串。
...
- 一个类对应一个资源集合（`modules/order/controller/OrderController` 管订单接口），一个方法 ≈ 一个用例。
...
- 一个 service 围绕一个业务主体聚合用例（`modules/order/service/OrderService`），**禁止**上帝 Service 一把抓。
...
| -------------- | ----------------------------- | ---------------------------- |
| 启动类            | 根包                            | 业务逻辑                         |
| 业务模块的 HTTP 入口 | `modules/order/controller`（`OrderController`） | 全局；一个 controller 塞多个资源集     |
| 用例编排/门面       | `modules/order/service`（`OrderService`）   | controller；mapper；上帝 Service |
| 业务模块本身        | `modules/` 下的 `order` / `user` 模块包       | 全局技术平铺后再套业务子包              |
| 单表持久化（基础）    | 全局 `mapper`（`extends BaseMapper`） | 写联表；模块内 mapper 塞单表          |
| 联表/复杂 SQL     | 发起模块的 `modules/order/mapper`（配 XML） | 全局单表 mapper；Service 里拼假 JOIN |
...
| 模块内再套第二层业务子包（`modules/order/order-item/…`）     | 模块内边界过度切分，分层职责分散         | 模块内按技术角色平铺（controller/service/mapper/dto） 各守其位 |
...
1. 它服务哪块**业务**？→ 进对应模块（`modules/` 下 `order` / `user`…），还是全局共享（`common` / `entity` / 全局单表 `mapper` 之一）？
...
- [ ] 业务模块统一收在顶层 `modules/` 下（`order`/`user`…），模块内按技术角色平铺；无全局技术层再套业务子包
...
- 「业务模块统一放顶层 modules/ 下、按模块分包、公共的放外面：entity 和单表 mapper 提到全局，业务模块再细分 controller/service/mapper/dto。」
