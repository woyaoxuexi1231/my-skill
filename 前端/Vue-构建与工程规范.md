...
> 范围：**环境变量、Vite 配置、依赖、性能预算、质量门禁、构建交付、工程卫生、测试底线**。  
> 本篇偏 **工程落地**，不写业务组件怎么画。
...
| 手动分包 | 稳定的大依赖（vue / 组件库 / 图表库）单独成 chunk，避免业务改动让整包缓存失效 |
...
- 用 **`browserslist`**（写在 `package.json` 或 `.browserslistrc`）声明支持范围，构建目标与 polyfill 策略据此推导。
- **禁止**既声明要支持旧浏览器、又在代码里随意用最新语法而不配目标。
- 需要兼容旧环境时靠构建降级，不在业务代码里手写两套分支。

### 3.4 禁止

- feature 内 `../../../` 深钻别的 feature 内部。
...
- `lint-staged` + `husky`（或等价方案）：提交前只跑改动文件的 lint / 格式化，**不做全量构建**（太慢会被绕过）。
- `commitlint`（可选）：约定式提交，便于生成 changelog。
- 钩子脚本本身要快；慢钩子最终都会被 `--no-verify` 绕过。
...
- 定期关注依赖告警（`pnpm audit` / Dependabot 等），但不在业务 PR 里顺手做大扫除。
...
不追求极致优化，但要有**可衡量的底线**，避免越做越慢还无人察觉。

### 6.1 核心指标目标（Core Web Vitals）
...
- 首屏 JS（gzip 后）设一个上限（如 ≤ 300KB 起步，按业务调整）并在 CI 或构建分析时可见。
- 单个 chunk 超过阈值时先查：是否把一个大库打进了首屏？是否忘了路由懒加载？
- 用 `rollup-plugin-visualizer`（或等价工具）定期看一次产物构成，**不为优化而优化**。

### 6.3 工程侧手段

- 路由级懒加载（架构已要求）；组件级按需引入大组件（图表、编辑器、地图）。
- 稳定的大依赖手动分包，业务代码变更不失效它们的缓存。
- 图片：现代格式（webp/avif）+ 合理尺寸；首屏图不懒加载，非首屏图懒加载。
- 字体：按需子集化，避免整包中文字体拖垮首屏。
...
- 带 hash 的产物可长缓存；`index.html` 与未被 hash 的入口文件禁止长缓存。
- 环境切换靠构建时 env 或运行时运维注入的**公开**配置，不靠改源码打多份业务分支。
- Docker / Nginx 示例若有：与 `base`、反代 `/api` 一致；本篇不绑定某一云厂商。

### 7.3 Sourcemap、上报与安全头

- 生产 sourcemap：**默认不公网直出**完整 map；若上传给错误监控，走私有通道，勿把 `.map` 裸挂在可匿名下载的 CDN 上（除非团队明文接受）。
- 上报 SDK 的 DSN 若必须进前端：视为「可公开的写入端点」，仍不在此塞其他密钥；事件需脱敏。
- **CSP**：生产建议由网关/Nginx 配 Content-Security-Policy（限制脚本来源）；前端避免 `eval`、无必要的 inline script。
- 配套头（部署侧）：`X-Content-Type-Options`、`Referrer-Policy`、按需 `frame-ancestors` / 点击劫持防护——不在业务代码里「假装设置」。
- 会话 Cookie 与 CSRF 的**前端侧约定**（不落 Web Storage、按约定带 CSRF 头）属模块规范；本篇只管部署侧头与密钥边界。
...
### 9.1 必须（一旦引入 Vitest / Testing Library 等）

- 测试放在约定位置（如 `*.spec.ts` / `__tests__`），禁止与无断言的临时脚本冒充测试。
- 遵守 AIR：**A**utomatic（自动 assert，禁用人肉看 console 验收）、**I**ndependent（不依赖执行顺序）、**R**epeatable（不依赖偶发外部环境；HTTP 可 mock）。
...
| 首屏塞入图表/编辑器等大依赖 | 路由或组件级懒加载 |
| 只测 happy path，不看产物体积 | 设预算 + 定期看产物构成 |
| 声称支持 IE 却不配构建目标 | 用 browserslist 声明并推导降级 |
...
- [ ] 浏览器目标由 `browserslist` 声明；大依赖已单独分包  
- [ ] lockfile 在库；未引入用不到的巨型依赖、无重复栈  

**性能**

- [ ] 路由级懒加载；首屏未塞入非必要大依赖  
- [ ] 核心指标有目标（LCP ≤ 2.5s / INP ≤ 200ms / CLS ≤ 0.1）  
- [ ] 产物体积有预算且可观测；图片/字体按需处理  
...
- [ ] `index.html` 未被长缓存；带 hash 产物可长缓存  

**打回语**

- 「密钥进了 VITE_，挪到后端。」  
- 「没跑 build 就说完成。」  
- 「api 写死内网 IP，改用 proxy + env。」  
- 「lockfile 没提交。」  
- 「图表库打进首屏了，改成按需引入。」  
- 「没声明 browserslist，构建目标不明确。」  
- 「生产公网挂了全量 sourcemap。」  
- 「改了契约但类型没过 typecheck。」  
